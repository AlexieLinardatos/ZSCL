...
# Ablation row 4: Full method — proportional replay + RTD (lambda=0.5).
# This is the same config as phase3_no_lora_11t_v3.sh but with uniform 1500
# iters per task (no per-task schedule) for a fair ablation comparison.
...
SAVE_PATH="ckpt/11task/ablation_full_v3"
mkdir -p "${SAVE_PATH}"
