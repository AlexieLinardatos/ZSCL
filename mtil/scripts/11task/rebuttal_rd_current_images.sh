...
# Rebuttal control C1 (Reviewer zvRY, Q2): are replay images essential to RD?
#
# Identical to phase3_no_lora_11t_v4.sh (the headline ExRD run) in every
# respect — same teacher, same 10,599 caption anchors, same lambda=0.3, same
# per-task iteration schedule, same 11k buffer with supervised replay CE —
# except that the RD term is evaluated on the CURRENT-TASK minibatch instead
# of on buffer exemplars.  The number of distilled images is held at
# replay_batch_size so the comparison is not confounded by batch size.
#
# Reading: if this matches ExRD, RD does not need the buffer and the buffer is
# only a label carrier.  If Transfer drops, the buffer contents are doing work
# that current-task images cannot do.
...
SAVE_PATH="/scratch/alexie/ckpt/11task/rebuttal_rd_current_images"
mkdir -p "${SAVE_PATH}"

EVAL_DATASETS="Aircraft,Caltech101,CIFAR100,DTD,EuroSAT,Flowers,Food,MNIST,OxfordPet,StanfordCars,SUN397,ImageNet"

echo "[`date`] Starting control C1 — RD on current-task images, frozen teacher"
