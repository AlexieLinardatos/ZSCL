...
# Adaptive ZSCL (NS4): dynamic hyperparameter scheduling on top of v4.
# Identical to phase3_no_lora_11t_v4.sh except for the dynamic schedule flags:
#   --zscl_loss_schedule ramp_up   : ramp ZSCL distillation weight 1.0 -> 1.5x
#                                    across tasks (more anchoring as drift
#                                    accumulates -> targets Transfer).
#   --lr_scale_by_class            : scale per-task lr by sqrt(classes/100),
#                                    clamped [0.5,1.5] (small datasets drift less).
#   --lambda_rtd_schedule warmup_cooldown : tent on lambda_RTD (ablation knob;
#                                    primarily helps Last). Base lambda_RTD=0.3.
# Baseline = v4 (paper config); v5/v6 were discarded.
...
SAVE_PATH="ckpt/11task/phase3_no_lora_v4dyn"
mkdir -p "${SAVE_PATH}"

EVAL_DATASETS="Aircraft,Caltech101,CIFAR100,DTD,EuroSAT,Flowers,Food,MNIST,OxfordPet,StanfordCars,SUN397,ImageNet"

echo "[`date`] Starting Phase 3 v4dyn — Adaptive ZSCL (ramp_up zscl + lr-by-class + warmup_cooldown lambda_RTD)"
