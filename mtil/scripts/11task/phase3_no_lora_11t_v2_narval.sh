...
# Phase 3 v2 — Narval variant
# Changes from phase3_no_lora_11t_v2.sh:
#   - gres: a100:1 instead of h100 MIG slice
#   - pip upgrade uses --no-index (no internet on Narval compute nodes)
#   - time: 48h (sufficient + better queue priority)
...
echo "[`date`] Starting Phase 3 v2 on Narval — proportional exemplars + per-task iterations"
