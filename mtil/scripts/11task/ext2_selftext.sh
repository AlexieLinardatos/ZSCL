#!/bin/bash
#SBATCH --job-name=11t_ext2_selftext
...
# Replay-buffer extension 2: SELF-TEXT ANCHORING
# Same config as phase3_no_lora_11t_v4.sh (random buffer), plus an extra loss
# that pulls each replay image's student embedding toward the FROZEN CLIP text
# embedding of its own true class name. Preserves the original image<->text
# geometry of the learned classes -> better zero-shot transfer retention.
# Knob: --lambda_replay_self_text (0 = off).
