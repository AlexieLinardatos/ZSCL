#!/bin/bash
#SBATCH --job-name=11t_ext3_recency
...
# Replay-buffer extension 3: RECENCY-ASYMMETRIC TEACHER DISTILLATION
# Same config as phase3_no_lora_11t_v4.sh (random buffer), but the replay
# teacher distillation is up-weighted for OLDER replayed tasks:
#   w(tid) = 1 + gamma * (max_tid - tid) / max_tid
# Older tasks (already consolidated) get stronger pressure to keep their
# original zero-shot geometry, trading a sliver of plasticity for transfer.
# Knobs: --replay_distill_recency  --replay_distill_recency_gamma.
...
srun python -m phase3.train_phase3 \
  ...
  --replay_distill_recency \
  --replay_distill_recency_gamma 1.0 \
