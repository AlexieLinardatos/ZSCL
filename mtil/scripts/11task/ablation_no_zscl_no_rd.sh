#!/bin/bash
#SBATCH --job-name=abl_nozscl_nord
...
# Substitution-claim baseline: replay only, no ZSCL distill, no RD.
# Loss: L_ce + L_l2 + L_replay_sup  (no L_zscl, no L_rtd).
# Tests how far Transfer falls when both anchor distillation
# mechanisms are removed. Pair with ablation_no_zscl_with_rd.sh.
# Matches v4 hyperparameters elsewhere (lr, schedule, budget) for
# direct comparability with "Ours (full)" in Table 1.
...
SAVE_PATH="ckpt/11task/ablation_no_zscl_no_rd"
mkdir -p "${SAVE_PATH}"
