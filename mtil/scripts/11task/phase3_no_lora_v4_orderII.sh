...
# Order II replication of v4 (our best run) for NeurIPS reviewer robustness check.
# Task ordering matches ZSCL/MoE-Adapters/GIFT Order II exactly:
#   StanfordCars -> Food -> MNIST -> OxfordPet -> Flowers -> SUN397
#   -> Aircraft -> Caltech101 -> DTD -> EuroSAT -> CIFAR100
#
# All hyperparameters are IDENTICAL to phase3_no_lora_11t_v4.sh:
#   lr=5e-6, lambda_RTD=0.3, replay_budget=11000, replay_batch_size=8,
#   replay_loss_weight=1.0, L2=1, ls=0.1, avg_freq=50, eval_interval=500.
# Per-task iteration counts are preserved per-dataset (same as v4):
#   StanfordCars:3000, Food:1500, MNIST:800, OxfordPet:1500, Flowers:1500,
#   SUN397:5000, Aircraft:3000, Caltech101:1000, DTD:1500, EuroSAT:1000,
#   CIFAR100:1500.
# No tuning was done for Order II; this is a direct robustness check.
...
SAVE_PATH="ckpt/11task/phase3_no_lora_v4_orderII"
mkdir -p "${SAVE_PATH}"
...
srun python -m phase3.train_phase3 \
  ...
  --task_iterations "StanfordCars:3000,Food:1500,MNIST:800,OxfordPet:1500,Flowers:1500,SUN397:5000,Aircraft:3000,Caltech101:1000,DTD:1500,EuroSAT:1000,CIFAR100:1500"
