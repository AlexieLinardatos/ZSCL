...
# Exemplar selection ablation: GCR (Gradient Coreset Replay)
# Same config as phase3_no_lora_11t_v4.sh, but the replay buffer picks
# samples greedily so the running mean last-layer gradient matches the
# full-data mean gradient (closed-form per-sample grad: (p - y) ⊗ φ).
...
SAVE_PATH="ckpt/11task/phase3_exemplar_gcr"
mkdir -p "${SAVE_PATH}"
