...
# Resume from OxfordPet.pth — only StanfordCars and SUN397 remain.
# OOM fix: replay_batch_size reduced from 16 -> 8.
# Resume logic is automatic: finetune_phase3.py reads replay_completed_tasks.txt
...
echo "[`date`] Resuming Phase 3 no-LoRA 11-task from OxfordPet (replay_batch_size=8)"
