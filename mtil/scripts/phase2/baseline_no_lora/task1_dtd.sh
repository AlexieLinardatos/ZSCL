...
TARGET_DATASET="DTD"
BASE_SAVE="ckpt/phase2.1/baseline_no_lora"
MODEL_PATH="${BASE_SAVE}/DTD_trained"
...
echo "[`date`] Stage 1: zero-shot eval (no LoRA)"
...
echo "[`date`] Stage 2: train DTD (no LoRA)"
srun python -m src.main \
  --train-mode=whole \
  --train-dataset="${TARGET_DATASET}" \
  --lr=1e-5 \
